...
- 化名登记与脱敏预览：登记受访人不愿公开的姓名/地名与化名，同一原名在各条轨道共用一个化名；开启脱敏预览后片段列表（正文、发言人、人物标注）显示化名，编辑框始终保留原稿。同一原名配了多个化名、或化名撞上原文里另一个人物的名字（正文、发言人或人物标注）时，脱敏导出会暂停并列出涉及片段，可直接跳转到对应片段。
- 化名登记随草稿保存在本机，登记、修改、删除都可撤销/重做（50 步）。
- 按当前轨道导出 SRT 字幕；脱敏预览开启时导出化名版本（文件名带“脱敏”后缀）。
